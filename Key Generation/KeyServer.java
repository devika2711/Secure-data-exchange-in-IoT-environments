...
import java.awt.event.ActionEvent;
import java.awt.event.ActionListener;
public class KeyServer extends JFrame{
	StandardPanel p1;
	JPanel p2,p3;
	JLabel title,l1;
	Font f1;
...
public void start(){
	try{
		...
		Object res[] = {"Key Generator Server Services Started"};
		dtm.addRow(res);
		while(true){
...
public KeyServer(){
	super("Key Generator Server Services Started");
	...
	l1 = new JLabel("Key Server Screen");
	l1.setFont(f1);
...
public static void main(String a[])throws Exception{
	...
	KeyServer cs = new KeyServer();
	cs.setVisible(true);
	cs.setExtendedState(JFrame.MAXIMIZED_BOTH);
	new KeyServerThread(cs);
}
}